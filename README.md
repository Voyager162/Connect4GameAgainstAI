...
- on Linux: `dist/Connect Four FX/` plus a zip like `Connect Four FX-<version>-linux-x64.zip`
...
The build process automatically detects your CPU architecture:
- **Apple Silicon Macs** (M1, M2, M3, etc.): Builds `macos-arm64` package
- **Intel Macs**: Builds `macos-x64` package

No extra configuration needed—just run the scripts as shown above!
...
- builds release zips for Windows x64, macOS Apple Silicon, and macOS Intel
- uploads the build artifacts
- automatically attaches the zips to a GitHub Release when you push a tag like `v1.0.0`

## macOS Note

The macOS app can be built and downloaded from GitHub Releases. Both Intel (x64) and Apple Silicon (ARM64) builds are published, so download the asset that matches your Mac.

**Security Warning:** The macOS app is not code signed or notarized yet. That means macOS may show an "unidentified developer" warning the first time someone opens it. To proceed:

1. Try opening from Finder (not Launchpad)
2. Right-click the app and select "Open"
3. Click "Open" in the security dialog

The app is safe to run—it's a pure Java application generated by `jpackage` with no external dependencies.
